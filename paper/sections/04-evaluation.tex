...
$\hat e_n$ is an unbiased estimator of $e$ with $\Var(\hat e_n) = e(1-e)/n$. Consequently $\SE(\hat e_n) = \sqrt{e(1-e)/n} \le \tfrac{1}{2\sqrt n}$, and by the de Moivre--Laplace central limit theorem an asymptotic $95\%$ confidence interval is $\hat e_n \pm 1.96\,\sqrt{\hat e_n(1-\hat e_n)/n}$.
\end{proposition}
\begin{proof}
Each $W_i$ is an independent draw of the random variable $W = \mathbf{1}\{\text{win}\} + \text{(fractional tie)}$, whose mean is $e$ by construction of the uniform sampling measure; ties contribute fractional mass so that $\E[W]$ equals the exact split-pot equity. The variance of a mean of $n$ i.i.d.\ bounded variables in $[0,1]$ with success probability $e$ is $e(1-e)/n$, maximised at $e = 1/2$.
\end{proof}
...
\caption{Standard error of the Monte Carlo equity estimator \eqref{eq:mcest} as a function of the number of simulated runouts, for true equities $e \in \{0.5, 0.25, 0.1\}$. The vertical lines mark the two simulation budgets of our Monte Carlo agent: the batch budget $n = 300$ (worst-case $\SE \approx 2.9$ percentage points) and the interactive budget $n = 1500$ ($\SE \approx 1.3$ points).}
...
\DontPrintSemicolon
\caption{Rule-based decision at one betting node (our \emph{rule agent}).}\label{alg:rule}
...
\If{$b = 0$}{
  \eIf{$s \ge \theta_{\text{open}}(\rho)$}{
    \eIf{$s \ge 0.62$ \textbf{and} $r < 0.35$}{emit pot-sized raise}{emit half-pot raise}\;
  }{check (fold-option hands check behind)\;}
}
\Else{
  $\theta \leftarrow b/(P+b) + 0.03 + 0.025\min(3,m) - \mathbb{1}[\rho \in \{\text{SB},\text{BB}\}]\cdot 0.02$\;
  \If{$s \ge 0.68$ \textbf{and} raises this street $< 3$ \textbf{and} $r < 0.55$}{emit 3-bet\;}
  \If{$s \ge \theta$}{call\;}
  \If{$\theta < 0.15$ \textbf{and} $s \ge 0.42$}{call (cheap peeling / set-mining)\;}
  \Return fold\;
}
\end{algorithm}
