...
Use the potential $\Phi^{t} = \sum_a \max(0, r^{t}(a))^2$. A Blackwell-condition computation shows $\Phi^{t} - \Phi^{t-1} \le \sum_a (v^t(a) - \bar v^t)^2 \le \Delta^{2}(|A|-1)$ at every step, and $R^{T} \le \sqrt{|A|\,\Phi^{T}}$ relates the potential to the regret. Chaining the two bounds and dividing by $T$ gives average regret $O(\Delta\sqrt{|A|/T})$; see~\cite{cesabianchi2006prediction} for the complete argument.
\end{proof}
